...
export function ProductCard({ product, index }: ProductCardProps) {
  return (
    ...
    >
      <Link to={product.link as any} className="block h-full w-full">
        <img
        ...
        <div className="absolute inset-x-0 bottom-0 pb-[38px] pl-12 pr-10">
          <div className="flex translate-y-3 items-start gap-[18px] transition-transform duration-[600ms] ease-out group-hover:translate-y-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              {product.description && (
                <div className="grid grid-rows-[0fr] opacity-0 transition-all duration-[500ms] ease-in-out group-hover:grid-rows-[1fr] group-hover:opacity-100">
                  <div className="overflow-hidden">
                    ...
                    <div
                      aria-hidden
                      className="mt-[18px] flex h-10 w-10 items-center justify-center rounded-[6px] bg-[#E53630] transition-colors duration-[300ms] ease-out hover:bg-[#c42f2a]"
                    >
                      <ArrowUpRight className="h-5 w-5 text-white transition-transform duration-[300ms] ease-out group-hover:rotate-45" />
                    </div>
                  </div>
                </div>
              )}
